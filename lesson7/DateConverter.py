...
def date_converter(language,datestr):
    index1 = datestr.find('/')
    index2 = datestr.find('/',index1+1)

    month = language[int(datestr[0:index1])]
    day = datestr[index1+1:index2]
    year = datestr[index2+1:]
    return day + ' ' + month + ' ' + year

def date_converter2(language,datestr):
